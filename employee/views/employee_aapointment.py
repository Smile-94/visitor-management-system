...
from employee.permission import AdminPassesTestMixin
...
class AppointmentView(LoginRequiredMixin, AdminPassesTestMixin, CreateView):
    model = Appointment
...
class AppointmentUpdateView(LoginRequiredMixin, AdminPassesTestMixin, UpdateView):
    model=Appointment
    form_class=AppointmentForm
    template_name='employee/add_appointment.html'
    success_url= reverse_lazy('employee:add_appointment')
...
class DeleteAppointmentView(LoginRequiredMixin, AdminPassesTestMixin, DeleteView):
    model= Appointment
...
class AppointmentCancelView(LoginRequiredMixin, AdminPassesTestMixin, UpdateView):
    model = Appointment
    form_class = AppointmentCancelForm
    template_name = 'employee/cancel_appointment.html'
    success_url = reverse_lazy('employee:add_appointment')
    ...
    def form_valid(self, form):
        if form.is_valid():
            cancel_obj=form.save(commit=False)
            cancel_obj.cancel_status=True
            cancel_obj.save()
            messages.success(self.request, "Cancel Appointment successful")
        return super().form_valid(form)
    
    def form_invalid(self, form):
        messages.error(self.request, "Cancel Appointment Unsuccessfull!")
        return super().form_invalid(form)
